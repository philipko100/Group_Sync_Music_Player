...
export default class Room extends Component {
    constructor(props) {
        ...
        this.authencateSpotify = this.authencateSpotify.bind(this);
        this.getRoomDetails();
    }
            ...
            .then((data) => {
                ...
                if(!data.status) {
                    fetch('/spotify/get-auth-url').then((response) => response.json())
                        .then((data) => {
                            window.location.replace(data.url); // redirect to spotify authentication url, then from there will call the callback function which will redirect back to our home page
                        });
                }
            });
    }
